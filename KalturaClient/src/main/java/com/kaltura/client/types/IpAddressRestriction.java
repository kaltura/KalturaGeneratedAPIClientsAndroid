...
public class IpAddressRestriction extends BaseRestriction {
    ...
    public static final Creator<IpAddressRestriction> CREATOR = new Creator<IpAddressRestriction>() {
        @Override
        public IpAddressRestriction createFromParcel(Parcel source) {
            return new IpAddressRestriction(source);
        }

        @Override
        public IpAddressRestriction[] newArray(int size) {
            return new IpAddressRestriction[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.ipAddressRestrictionType == null ? -1 : this.ipAddressRestrictionType.ordinal());
        dest.writeString(this.ipAddressList);
    }

    public IpAddressRestriction(Parcel in) {
        super(in);
        int tmpIpAddressRestrictionType = in.readInt();
        this.ipAddressRestrictionType = tmpIpAddressRestrictionType == -1 ? null : IpAddressRestrictionType.values()[tmpIpAddressRestrictionType];
        this.ipAddressList = in.readString();
    }
}
