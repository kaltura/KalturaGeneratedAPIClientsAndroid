...
public class ModerationFlag extends ObjectBase {
    ...
    public static final Creator<ModerationFlag> CREATOR = new Creator<ModerationFlag>() {
        @Override
        public ModerationFlag createFromParcel(Parcel source) {
            return new ModerationFlag(source);
        }

        @Override
        public ModerationFlag[] newArray(int size) {
            return new ModerationFlag[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.moderationObjectType == null ? -1 : this.moderationObjectType.ordinal());
        dest.writeString(this.flaggedEntryId);
        dest.writeString(this.flaggedUserId);
        dest.writeInt(this.status == null ? -1 : this.status.ordinal());
        dest.writeString(this.comments);
        dest.writeInt(this.flagType == null ? -1 : this.flagType.ordinal());
        dest.writeValue(this.createdAt);
        dest.writeValue(this.updatedAt);
    }

    public ModerationFlag(Parcel in) {
        super(in);
        this.id = (Integer)in.readValue(Integer.class.getClassLoader());
        this.partnerId = (Integer)in.readValue(Integer.class.getClassLoader());
        this.userId = in.readString();
        int tmpModerationObjectType = in.readInt();
        this.moderationObjectType = tmpModerationObjectType == -1 ? null : ModerationObjectType.values()[tmpModerationObjectType];
        this.flaggedEntryId = in.readString();
        this.flaggedUserId = in.readString();
        int tmpStatus = in.readInt();
        this.status = tmpStatus == -1 ? null : ModerationFlagStatus.values()[tmpStatus];
