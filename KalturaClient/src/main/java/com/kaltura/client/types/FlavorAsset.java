...
public class FlavorAsset extends Asset {
    ...
    public static final Creator<FlavorAsset> CREATOR = new Creator<FlavorAsset>() {
        @Override
        public FlavorAsset createFromParcel(Parcel source) {
            return new FlavorAsset(source);
        }

        @Override
        public FlavorAsset[] newArray(int size) {
            return new FlavorAsset[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.isOriginal);
        dest.writeValue(this.isWeb);
    ...
    public FlavorAsset(Parcel in) {
        ...
        this.isOriginal = (Boolean)in.readValue(Boolean.class.getClassLoader());
        this.isWeb = (Boolean)in.readValue(Boolean.class.getClassLoader());
        this.containerFormat = in.readString();
        this.videoCodecId = in.readString();
        int tmpStatus = in.readInt();
        this.status = tmpStatus == -1 ? null : FlavorAssetStatus.values()[tmpStatus];
