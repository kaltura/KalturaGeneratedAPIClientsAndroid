...
public class ExtendingItemMrssParameter extends ObjectBase {
    ...
    public static final Creator<ExtendingItemMrssParameter> CREATOR = new Creator<ExtendingItemMrssParameter>() {
        @Override
        public ExtendingItemMrssParameter createFromParcel(Parcel source) {
            return new ExtendingItemMrssParameter(source);
        }

        @Override
        public ExtendingItemMrssParameter[] newArray(int size) {
            return new ExtendingItemMrssParameter[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.extensionMode == null ? -1 : this.extensionMode.ordinal());
    }

    public ExtendingItemMrssParameter(Parcel in) {
        super(in);
        this.xpath = in.readString();
        this.identifier = in.readParcelable(ObjectIdentifier.class.getClassLoader());
        int tmpExtensionMode = in.readInt();
        this.extensionMode = tmpExtensionMode == -1 ? null : MrssExtensionMode.values()[tmpExtensionMode];
    }
}
