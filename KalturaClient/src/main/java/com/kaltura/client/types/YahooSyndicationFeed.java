...
public class YahooSyndicationFeed extends BaseSyndicationFeed {
    ...
    public static final Creator<YahooSyndicationFeed> CREATOR = new Creator<YahooSyndicationFeed>() {
        @Override
        public YahooSyndicationFeed createFromParcel(Parcel source) {
            return new YahooSyndicationFeed(source);
        }

        @Override
        public YahooSyndicationFeed[] newArray(int size) {
            return new YahooSyndicationFeed[size];
        }
    };
    ...
    public YahooSyndicationFeed(Parcel in) {
        super(in);
        int tmpCategory = in.readInt();
        this.category = tmpCategory == -1 ? null : YahooSyndicationFeedCategories.values()[tmpCategory];
        int tmpAdultContent = in.readInt();
        this.adultContent = tmpAdultContent == -1 ? null : YahooSyndicationFeedAdultValues.values()[tmpAdultContent];
