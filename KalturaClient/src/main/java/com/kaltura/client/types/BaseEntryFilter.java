...
public class BaseEntryFilter extends BaseEntryBaseFilter {
    ...
    public static final Creator<BaseEntryFilter> CREATOR = new Creator<BaseEntryFilter>() {
        @Override
        public BaseEntryFilter createFromParcel(Parcel source) {
            return new BaseEntryFilter(source);
        }

        @Override
        public BaseEntryFilter[] newArray(int size) {
            return new BaseEntryFilter[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.isRoot == null ? -1 : this.isRoot.ordinal());
    ...
    public BaseEntryFilter(Parcel in) {
        ...
        int tmpIsRoot = in.readInt();
        this.isRoot = tmpIsRoot == -1 ? null : Boolean.values()[tmpIsRoot];
