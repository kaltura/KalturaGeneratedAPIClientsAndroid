...
public class ContentDistributionSearchItem extends SearchItem {
    ...
    public static final Creator<ContentDistributionSearchItem> CREATOR = new Creator<ContentDistributionSearchItem>() {
        @Override
        public ContentDistributionSearchItem createFromParcel(Parcel source) {
            return new ContentDistributionSearchItem(source);
        }

        @Override
        public ContentDistributionSearchItem[] newArray(int size) {
            return new ContentDistributionSearchItem[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.noDistributionProfiles);
        dest.writeValue(this.distributionProfileId);
        dest.writeInt(this.distributionSunStatus == null ? -1 : this.distributionSunStatus.ordinal());
        dest.writeInt(this.entryDistributionFlag == null ? -1 : this.entryDistributionFlag.ordinal());
        dest.writeInt(this.entryDistributionStatus == null ? -1 : this.entryDistributionStatus.ordinal());
        dest.writeValue(this.hasEntryDistributionValidationErrors);
        dest.writeString(this.entryDistributionValidationErrors);
    }

    public ContentDistributionSearchItem(Parcel in) {
        super(in);
        this.noDistributionProfiles = (Boolean)in.readValue(Boolean.class.getClassLoader());
        this.distributionProfileId = (Integer)in.readValue(Integer.class.getClassLoader());
        int tmpDistributionSunStatus = in.readInt();
        this.distributionSunStatus = tmpDistributionSunStatus == -1 ? null : EntryDistributionSunStatus.values()[tmpDistributionSunStatus];
        int tmpEntryDistributionFlag = in.readInt();
        this.entryDistributionFlag = tmpEntryDistributionFlag == -1 ? null : EntryDistributionFlag.values()[tmpEntryDistributionFlag];
        int tmpEntryDistributionStatus = in.readInt();
        this.entryDistributionStatus = tmpEntryDistributionStatus == -1 ? null : EntryDistributionStatus.values()[tmpEntryDistributionStatus];
        this.hasEntryDistributionValidationErrors = (Boolean)in.readValue(Boolean.class.getClassLoader());
        this.entryDistributionValidationErrors = in.readString();
    }
}
