...
public class DeleteEntryFlavorsObjectTask extends ObjectTask {
    ...
    public static final Creator<DeleteEntryFlavorsObjectTask> CREATOR = new Creator<DeleteEntryFlavorsObjectTask>() {
        @Override
        public DeleteEntryFlavorsObjectTask createFromParcel(Parcel source) {
            return new DeleteEntryFlavorsObjectTask(source);
        }

        @Override
        public DeleteEntryFlavorsObjectTask[] newArray(int size) {
            return new DeleteEntryFlavorsObjectTask[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.deleteType == null ? -1 : this.deleteType.ordinal());
        dest.writeString(this.flavorParamsIds);
    }

    public DeleteEntryFlavorsObjectTask(Parcel in) {
        super(in);
        int tmpDeleteType = in.readInt();
        this.deleteType = tmpDeleteType == -1 ? null : DeleteFlavorsLogicType.values()[tmpDeleteType];
