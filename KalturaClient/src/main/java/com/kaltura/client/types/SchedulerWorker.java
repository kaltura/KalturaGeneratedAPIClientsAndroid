...
public class SchedulerWorker extends ObjectBase {
    ...
    public static final Creator<SchedulerWorker> CREATOR = new Creator<SchedulerWorker>() {
        @Override
        public SchedulerWorker createFromParcel(Parcel source) {
            return new SchedulerWorker(source);
        }

        @Override
        public SchedulerWorker[] newArray(int size) {
            return new SchedulerWorker[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.schedulerConfiguredId);
        ...
        if(this.lockedJobs != null) {
            dest.writeInt(this.lockedJobs.size());
            dest.writeList(this.lockedJobs);
        } else {
            dest.writeInt(-1);
        }
        dest.writeValue(this.avgWait);
        dest.writeValue(this.avgWork);
        dest.writeValue(this.lastStatus);
        dest.writeString(this.lastStatusStr);
    }

    public SchedulerWorker(Parcel in) {
        super(in);
        ...
        this.schedulerConfiguredId = (Integer)in.readValue(Integer.class.getClassLoader());
        int tmpType = in.readInt();
        this.type = tmpType == -1 ? null : BatchJobType.values()[tmpType];
        ...
        int lockedJobsSize = in.readInt();
        if( lockedJobsSize > -1) {
            this.lockedJobs = new ArrayList<>();
            in.readList(this.lockedJobs, BatchJob.class.getClassLoader());
        }
        this.avgWait = (Integer)in.readValue(Integer.class.getClassLoader());
        this.avgWork = (Integer)in.readValue(Integer.class.getClassLoader());
