...
public class AssetParams extends ObjectBase {
    ...
    public static final Creator<AssetParams> CREATOR = new Creator<AssetParams>() {
        @Override
        public AssetParams createFromParcel(Parcel source) {
            return new AssetParams(source);
        }

        @Override
        public AssetParams[] newArray(int size) {
            return new AssetParams[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.isSystemDefault);
        dest.writeString(this.tags);
        if(this.requiredPermissions != null) {
            dest.writeInt(this.requiredPermissions.size());
            dest.writeList(this.requiredPermissions);
        } else {
            dest.writeInt(-1);
        }
        dest.writeValue(this.sourceRemoteStorageProfileId);
        dest.writeValue(this.remoteStorageProfileIds);
        dest.writeInt(this.mediaParserType == null ? -1 : this.mediaParserType.ordinal());
        dest.writeString(this.sourceAssetParamsIds);
    }

    public AssetParams(Parcel in) {
        super(in);
        ...
        if( requiredPermissionsSize > -1) {
            ...
            in.readList(this.requiredPermissions, StringHolder.class.getClassLoader());
        }
        this.sourceRemoteStorageProfileId = (Integer)in.readValue(Integer.class.getClassLoader());
        this.remoteStorageProfileIds = (Integer)in.readValue(Integer.class.getClassLoader());
        ...
        this.sourceAssetParamsIds = in.readString();
    }
}
