...
public class YahooDistributionProfile extends ConfigurableDistributionProfile {
    ...
    public static final Creator<YahooDistributionProfile> CREATOR = new Creator<YahooDistributionProfile>() {
        @Override
        public YahooDistributionProfile createFromParcel(Parcel source) {
            return new YahooDistributionProfile(source);
        }

        @Override
        public YahooDistributionProfile[] newArray(int size) {
            return new YahooDistributionProfile[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.contactTelephone);
        dest.writeString(this.contactEmail);
        dest.writeInt(this.processFeed == null ? -1 : this.processFeed.ordinal());
    }

    public YahooDistributionProfile(Parcel in) {
        super(in);
        this.ftpPath = in.readString();
        this.ftpUsername = in.readString();
        this.ftpPassword = in.readString();
        this.ftpHost = in.readString();
        this.contactTelephone = in.readString();
        this.contactEmail = in.readString();
        int tmpProcessFeed = in.readInt();
        this.processFeed = tmpProcessFeed == -1 ? null : YahooDistributionProcessFeedActionStatus.values()[tmpProcessFeed];
    }
}
