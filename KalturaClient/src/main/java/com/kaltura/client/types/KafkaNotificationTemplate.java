...
public class KafkaNotificationTemplate extends EventNotificationTemplate {
    ...
    public static final Creator<KafkaNotificationTemplate> CREATOR = new Creator<KafkaNotificationTemplate>() {
        @Override
        public KafkaNotificationTemplate createFromParcel(Parcel source) {
            return new KafkaNotificationTemplate(source);
        }

        @Override
        public KafkaNotificationTemplate[] newArray(int size) {
            return new KafkaNotificationTemplate[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.partitionKey);
        dest.writeInt(this.messageFormat == null ? -1 : this.messageFormat.ordinal());
        dest.writeString(this.apiObjectType);
        dest.writeString(this.responseProfileSystemName);
        dest.writeString(this.requiresPermissions);
    }

    public KafkaNotificationTemplate(Parcel in) {
        super(in);
        this.topicName = in.readString();
        this.partitionKey = in.readString();
        int tmpMessageFormat = in.readInt();
        this.messageFormat = tmpMessageFormat == -1 ? null : KafkaNotificationFormat.values()[tmpMessageFormat];
        this.apiObjectType = in.readString();
        this.responseProfileSystemName = in.readString();
        this.requiresPermissions = in.readString();
    }
}
