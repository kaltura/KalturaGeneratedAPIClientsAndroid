...
public class UiConfV2Redirect extends ObjectBase {
    ...
    public static final Creator<UiConfV2Redirect> CREATOR = new Creator<UiConfV2Redirect>() {
        @Override
        public UiConfV2Redirect createFromParcel(Parcel source) {
            return new UiConfV2Redirect(source);
        }

        @Override
        public UiConfV2Redirect[] newArray(int size) {
            return new UiConfV2Redirect[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.v7id);
        dest.writeValue(this.isApproved);
        dest.writeValue(this.translatePlugins);
    }

    public UiConfV2Redirect(Parcel in) {
        super(in);
        this.v7id = (Integer)in.readValue(Integer.class.getClassLoader());
        this.isApproved = (Boolean)in.readValue(Boolean.class.getClassLoader());
        this.translatePlugins = (Boolean)in.readValue(Boolean.class.getClassLoader());
    }
}
