...
public class DistributionJobData extends JobData {
    ...
    public static final Creator<DistributionJobData> CREATOR = new Creator<DistributionJobData>() {
        @Override
        public DistributionJobData createFromParcel(Parcel source) {
            return new DistributionJobData(source);
        }

        @Override
        public DistributionJobData[] newArray(int size) {
            return new DistributionJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeParcelable(this.distributionProfile, flags);
        dest.writeValue(this.entryDistributionId);
        dest.writeParcelable(this.entryDistribution, flags);
        dest.writeString(this.remoteId);
        dest.writeInt(this.providerType == null ? -1 : this.providerType.ordinal());
        dest.writeParcelable(this.providerData, flags);
        dest.writeString(this.results);
        dest.writeString(this.sentData);
    ...
    public DistributionJobData(Parcel in) {
        super(in);
        this.distributionProfileId = (Integer)in.readValue(Integer.class.getClassLoader());
        this.distributionProfile = in.readParcelable(DistributionProfile.class.getClassLoader());
        this.entryDistributionId = (Integer)in.readValue(Integer.class.getClassLoader());
        this.entryDistribution = in.readParcelable(EntryDistribution.class.getClassLoader());
        ...
        this.providerData = in.readParcelable(DistributionJobProviderData.class.getClassLoader());
        this.results = in.readString();
        this.sentData = in.readString();
        int mediaFilesSize = in.readInt();
        if( mediaFilesSize > -1) {
            this.mediaFiles = new ArrayList<>();
            in.readList(this.mediaFiles, DistributionRemoteMediaFile.class.getClassLoader());
        }
    }
}
