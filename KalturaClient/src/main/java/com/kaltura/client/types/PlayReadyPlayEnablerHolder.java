...
public class PlayReadyPlayEnablerHolder extends ObjectBase {
    ...
    public static final Creator<PlayReadyPlayEnablerHolder> CREATOR = new Creator<PlayReadyPlayEnablerHolder>() {
        @Override
        public PlayReadyPlayEnablerHolder createFromParcel(Parcel source) {
            return new PlayReadyPlayEnablerHolder(source);
        }

        @Override
        public PlayReadyPlayEnablerHolder[] newArray(int size) {
            return new PlayReadyPlayEnablerHolder[size];
        }
    };
    ...
    public PlayReadyPlayEnablerHolder(Parcel in) {
        super(in);
        int tmpType = in.readInt();
        this.type = tmpType == -1 ? null : PlayReadyPlayEnablerType.values()[tmpType];
    }
}
