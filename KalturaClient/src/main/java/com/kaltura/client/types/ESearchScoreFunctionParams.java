...
public class ESearchScoreFunctionParams extends ObjectBase {
    ...
    public static final Creator<ESearchScoreFunctionParams> CREATOR = new Creator<ESearchScoreFunctionParams>() {
        @Override
        public ESearchScoreFunctionParams createFromParcel(Parcel source) {
            return new ESearchScoreFunctionParams(source);
        }

        @Override
        public ESearchScoreFunctionParams[] newArray(int size) {
            return new ESearchScoreFunctionParams[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.decayAlgorithm == null ? -1 : this.decayAlgorithm.ordinal());
        dest.writeInt(this.functionField == null ? -1 : this.functionField.ordinal());
        dest.writeInt(this.boostMode == null ? -1 : this.boostMode.ordinal());
        dest.writeInt(this.origin == null ? -1 : this.origin.ordinal());
        dest.writeValue(this.weight);
        dest.writeString(this.scale);
        dest.writeValue(this.decay);
    }

    public ESearchScoreFunctionParams(Parcel in) {
        super(in);
        int tmpDecayAlgorithm = in.readInt();
        this.decayAlgorithm = tmpDecayAlgorithm == -1 ? null : ESearchScoreFunctionDecayAlgorithm.values()[tmpDecayAlgorithm];
        int tmpFunctionField = in.readInt();
        this.functionField = tmpFunctionField == -1 ? null : ESearchScoreFunctionField.values()[tmpFunctionField];
        int tmpBoostMode = in.readInt();
        this.boostMode = tmpBoostMode == -1 ? null : ESearchScoreFunctionBoostMode.values()[tmpBoostMode];
        int tmpOrigin = in.readInt();
        this.origin = tmpOrigin == -1 ? null : ESearchScoreFunctionOrigin.values()[tmpOrigin];
        this.weight = (Double)in.readValue(Double.class.getClassLoader());
        this.scale = in.readString();
        this.decay = (Double)in.readValue(Double.class.getClassLoader());
    }
}
