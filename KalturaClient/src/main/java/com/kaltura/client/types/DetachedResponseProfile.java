...
public class DetachedResponseProfile extends BaseResponseProfile {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.mappings != null) {
            dest.writeInt(this.mappings.size());
            dest.writeList(this.mappings);
        } else {
            dest.writeInt(-1);
        }
    }
    ...
    public DetachedResponseProfile(Parcel in) {
        ...
        this.type = tmpType == -1 ? null : ResponseProfileType.values()[tmpType];
        this.fields = in.readString();
        this.filter = in.readParcelable(RelatedFilter.class.getClassLoader());
        this.pager = in.readParcelable(FilterPager.class.getClassLoader());
        ...
        if( mappingsSize > -1) {
            ...
            in.readList(this.mappings, ResponseProfileMapping.class.getClassLoader());
        }
    }
}
