...
public class ConvertLiveSegmentJobData extends JobData {
    ...
    public static final Creator<ConvertLiveSegmentJobData> CREATOR = new Creator<ConvertLiveSegmentJobData>() {
        @Override
        public ConvertLiveSegmentJobData createFromParcel(Parcel source) {
            return new ConvertLiveSegmentJobData(source);
        }

        @Override
        public ConvertLiveSegmentJobData[] newArray(int size) {
            return new ConvertLiveSegmentJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.mediaServerIndex == null ? -1 : this.mediaServerIndex.ordinal());
        dest.writeValue(this.fileIndex);
        dest.writeString(this.srcFilePath);
        dest.writeString(this.destFilePath);
        dest.writeValue(this.endTime);
        dest.writeString(this.destDataFilePath);
    }

    public ConvertLiveSegmentJobData(Parcel in) {
        super(in);
        this.entryId = in.readString();
        this.assetId = in.readString();
        int tmpMediaServerIndex = in.readInt();
        this.mediaServerIndex = tmpMediaServerIndex == -1 ? null : EntryServerNodeType.values()[tmpMediaServerIndex];
        this.fileIndex = (Integer)in.readValue(Integer.class.getClassLoader());
        this.srcFilePath = in.readString();
        this.destFilePath = in.readString();
        this.endTime = (Double)in.readValue(Double.class.getClassLoader());
        this.destDataFilePath = in.readString();
    }
}
