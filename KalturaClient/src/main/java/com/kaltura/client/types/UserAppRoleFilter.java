...
public class UserAppRoleFilter extends UserAppRoleBaseFilter {
    ...
    public static final Creator<UserAppRoleFilter> CREATOR = new Creator<UserAppRoleFilter>() {
        @Override
        public UserAppRoleFilter createFromParcel(Parcel source) {
            return new UserAppRoleFilter(source);
        }

        @Override
        public UserAppRoleFilter[] newArray(int size) {
            return new UserAppRoleFilter[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.userStatusEqual == null ? -1 : this.userStatusEqual.ordinal());
        dest.writeString(this.userStatusIn);
    }

    public UserAppRoleFilter(Parcel in) {
        super(in);
        this.userIdEqual = in.readString();
        this.userIdIn = in.readString();
        int tmpUserStatusEqual = in.readInt();
        this.userStatusEqual = tmpUserStatusEqual == -1 ? null : UserStatus.values()[tmpUserStatusEqual];
        this.userStatusIn = in.readString();
    }
}
