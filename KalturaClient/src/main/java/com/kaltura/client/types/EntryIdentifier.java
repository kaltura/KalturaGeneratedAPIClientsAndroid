...
public class EntryIdentifier extends ObjectIdentifier {
    ...
    public static final Creator<EntryIdentifier> CREATOR = new Creator<EntryIdentifier>() {
        @Override
        public EntryIdentifier createFromParcel(Parcel source) {
            return new EntryIdentifier(source);
        }

        @Override
        public EntryIdentifier[] newArray(int size) {
            return new EntryIdentifier[size];
        }
    };
    ...
    public EntryIdentifier(Parcel in) {
        super(in);
        int tmpIdentifier = in.readInt();
        this.identifier = tmpIdentifier == -1 ? null : EntryIdentifierField.values()[tmpIdentifier];
    }
}
