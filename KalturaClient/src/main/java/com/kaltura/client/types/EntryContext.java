...
public class EntryContext extends Context {
    ...
    public static final Creator<EntryContext> CREATOR = new Creator<EntryContext>() {
        @Override
        public EntryContext createFromParcel(Parcel source) {
            return new EntryContext(source);
        }

        @Override
        public EntryContext[] newArray(int size) {
            return new EntryContext[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.followEntryRedirect == null ? -1 : this.followEntryRedirect.ordinal());
    }

    public EntryContext(Parcel in) {
        super(in);
        this.entryId = in.readString();
        int tmpFollowEntryRedirect = in.readInt();
        this.followEntryRedirect = tmpFollowEntryRedirect == -1 ? null : Boolean.values()[tmpFollowEntryRedirect];
    }
}
