...
public class AdCuePoint extends CuePoint {
    ...
    public static final Creator<AdCuePoint> CREATOR = new Creator<AdCuePoint>() {
        @Override
        public AdCuePoint createFromParcel(Parcel source) {
            return new AdCuePoint(source);
        }

        @Override
        public AdCuePoint[] newArray(int size) {
            return new AdCuePoint[size];
        }
    };
    ...
    public AdCuePoint(Parcel in) {
        super(in);
        int tmpProtocolType = in.readInt();
        this.protocolType = tmpProtocolType == -1 ? null : AdProtocolType.values()[tmpProtocolType];
