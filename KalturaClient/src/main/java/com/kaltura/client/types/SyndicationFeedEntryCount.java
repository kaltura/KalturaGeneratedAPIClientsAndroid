...
public class SyndicationFeedEntryCount extends ObjectBase {
    ...
    public static final Creator<SyndicationFeedEntryCount> CREATOR = new Creator<SyndicationFeedEntryCount>() {
        @Override
        public SyndicationFeedEntryCount createFromParcel(Parcel source) {
            return new SyndicationFeedEntryCount(source);
        }

        @Override
        public SyndicationFeedEntryCount[] newArray(int size) {
            return new SyndicationFeedEntryCount[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.totalEntryCount);
        dest.writeValue(this.actualEntryCount);
        dest.writeValue(this.requireTranscodingCount);
    }

    public SyndicationFeedEntryCount(Parcel in) {
        super(in);
        this.totalEntryCount = (Integer)in.readValue(Integer.class.getClassLoader());
        this.actualEntryCount = (Integer)in.readValue(Integer.class.getClassLoader());
        this.requireTranscodingCount = (Integer)in.readValue(Integer.class.getClassLoader());
    }
}
