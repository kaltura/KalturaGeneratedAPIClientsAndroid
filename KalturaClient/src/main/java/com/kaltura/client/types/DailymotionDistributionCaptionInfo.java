...
public class DailymotionDistributionCaptionInfo extends ObjectBase {
    ...
    public static final Creator<DailymotionDistributionCaptionInfo> CREATOR = new Creator<DailymotionDistributionCaptionInfo>() {
        @Override
        public DailymotionDistributionCaptionInfo createFromParcel(Parcel source) {
            return new DailymotionDistributionCaptionInfo(source);
        }

        @Override
        public DailymotionDistributionCaptionInfo[] newArray(int size) {
            return new DailymotionDistributionCaptionInfo[size];
        }
    };
    ...
    public DailymotionDistributionCaptionInfo(Parcel in) {
        super(in);
        this.language = in.readString();
        this.filePath = in.readString();
        this.remoteId = in.readString();
        int tmpAction = in.readInt();
        this.action = tmpAction == -1 ? null : DailymotionDistributionCaptionAction.values()[tmpAction];
        this.version = in.readString();
        this.assetId = in.readString();
        int tmpFormat = in.readInt();
        this.format = tmpFormat == -1 ? null : DailymotionDistributionCaptionFormat.values()[tmpFormat];
    }
}
