...
public class UiConfTypeInfo extends ObjectBase {
    ...
    public static final Creator<UiConfTypeInfo> CREATOR = new Creator<UiConfTypeInfo>() {
        @Override
        public UiConfTypeInfo createFromParcel(Parcel source) {
            return new UiConfTypeInfo(source);
        }

        @Override
        public UiConfTypeInfo[] newArray(int size) {
            return new UiConfTypeInfo[size];
        }
    };
    ...
    public UiConfTypeInfo(Parcel in) {
        super(in);
        int tmpType = in.readInt();
        this.type = tmpType == -1 ? null : UiConfObjType.values()[tmpType];
        int versionsSize = in.readInt();
        if( versionsSize > -1) {
            this.versions = new ArrayList<>();
            in.readList(this.versions, StringHolder.class.getClassLoader());
        }
