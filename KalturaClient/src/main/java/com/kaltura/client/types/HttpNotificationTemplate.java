...
public class HttpNotificationTemplate extends EventNotificationTemplate {
    ...
    public static final Creator<HttpNotificationTemplate> CREATOR = new Creator<HttpNotificationTemplate>() {
        @Override
        public HttpNotificationTemplate createFromParcel(Parcel source) {
            return new HttpNotificationTemplate(source);
        }

        @Override
        public HttpNotificationTemplate[] newArray(int size) {
            return new HttpNotificationTemplate[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.secureHashingAlgo == null ? -1 : this.secureHashingAlgo.ordinal());
    }

    public HttpNotificationTemplate(Parcel in) {
        super(in);
        this.url = in.readString();
        int tmpMethod = in.readInt();
        this.method = tmpMethod == -1 ? null : HttpNotificationMethod.values()[tmpMethod];
        this.data = in.readParcelable(HttpNotificationData.class.getClassLoader());
        ...
        int tmpSecureHashingAlgo = in.readInt();
        this.secureHashingAlgo = tmpSecureHashingAlgo == -1 ? null : SecureHashingAlgo.values()[tmpSecureHashingAlgo];
    }
}
