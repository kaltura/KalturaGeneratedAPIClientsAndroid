...
public class AccessControlScope extends ObjectBase {
    ...
    public static final Creator<AccessControlScope> CREATOR = new Creator<AccessControlScope>() {
        @Override
        public AccessControlScope createFromParcel(Parcel source) {
            return new AccessControlScope(source);
        }

        @Override
        public AccessControlScope[] newArray(int size) {
            return new AccessControlScope[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.hashes != null) {
            dest.writeInt(this.hashes.size());
            dest.writeList(this.hashes);
        } else {
            dest.writeInt(-1);
        }
    }

    public AccessControlScope(Parcel in) {
        super(in);
        ...
        if( contextsSize > -1) {
            ...
            in.readList(this.contexts, AccessControlContextTypeHolder.class.getClassLoader());
        }
        int hashesSize = in.readInt();
        if( hashesSize > -1) {
            this.hashes = new ArrayList<>();
            in.readList(this.hashes, KeyValue.class.getClassLoader());
        }
    }
}
