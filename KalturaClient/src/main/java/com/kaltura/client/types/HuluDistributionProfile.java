...
public class HuluDistributionProfile extends ConfigurableDistributionProfile {
    ...
    public static final Creator<HuluDistributionProfile> CREATOR = new Creator<HuluDistributionProfile>() {
        @Override
        public HuluDistributionProfile createFromParcel(Parcel source) {
            return new HuluDistributionProfile(source);
        }

        @Override
        public HuluDistributionProfile[] newArray(int size) {
            return new HuluDistributionProfile[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.sftpPass);
        dest.writeString(this.seriesChannel);
        dest.writeString(this.seriesPrimaryCategory);
        if(this.seriesAdditionalCategories != null) {
            dest.writeInt(this.seriesAdditionalCategories.size());
            dest.writeList(this.seriesAdditionalCategories);
        } else {
            dest.writeInt(-1);
        }
        dest.writeString(this.seasonNumber);
        dest.writeString(this.seasonSynopsis);
        dest.writeString(this.seasonTuneInInformation);
        dest.writeString(this.videoMediaType);
        dest.writeValue(this.disableEpisodeNumberCustomValidation);
        dest.writeInt(this.protocol == null ? -1 : this.protocol.ordinal());
        dest.writeString(this.asperaHost);
        dest.writeString(this.asperaLogin);
        dest.writeString(this.asperaPass);
        dest.writeValue(this.port);
        dest.writeString(this.passphrase);
        dest.writeString(this.asperaPublicKey);
        dest.writeString(this.asperaPrivateKey);
    }

    public HuluDistributionProfile(Parcel in) {
        super(in);
        this.sftpHost = in.readString();
        this.sftpLogin = in.readString();
        this.sftpPass = in.readString();
        this.seriesChannel = in.readString();
        this.seriesPrimaryCategory = in.readString();
        int seriesAdditionalCategoriesSize = in.readInt();
        if( seriesAdditionalCategoriesSize > -1) {
            this.seriesAdditionalCategories = new ArrayList<>();
            in.readList(this.seriesAdditionalCategories, StringHolder.class.getClassLoader());
        }
        this.seasonNumber = in.readString();
        this.seasonSynopsis = in.readString();
        this.seasonTuneInInformation = in.readString();
        this.videoMediaType = in.readString();
        this.disableEpisodeNumberCustomValidation = (Boolean)in.readValue(Boolean.class.getClassLoader());
        int tmpProtocol = in.readInt();
        this.protocol = tmpProtocol == -1 ? null : DistributionProtocol.values()[tmpProtocol];
        this.asperaHost = in.readString();
        this.asperaLogin = in.readString();
        this.asperaPass = in.readString();
        this.port = (Integer)in.readValue(Integer.class.getClassLoader());
        this.passphrase = in.readString();
        this.asperaPublicKey = in.readString();
        this.asperaPrivateKey = in.readString();
    }
}
