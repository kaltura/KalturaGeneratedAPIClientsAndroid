...
public class NotificationJobData extends JobData {
    ...
    public static final Creator<NotificationJobData> CREATOR = new Creator<NotificationJobData>() {
        @Override
        public NotificationJobData createFromParcel(Parcel source) {
            return new NotificationJobData(source);
        }

        @Override
        public NotificationJobData[] newArray(int size) {
            return new NotificationJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.typeAsString);
        dest.writeString(this.objectId);
        dest.writeInt(this.status == null ? -1 : this.status.ordinal());
        dest.writeString(this.data);
        dest.writeValue(this.numberOfAttempts);
        dest.writeString(this.notificationResult);
        dest.writeInt(this.objType == null ? -1 : this.objType.ordinal());
    }

    public NotificationJobData(Parcel in) {
        super(in);
        this.userId = in.readString();
        int tmpType = in.readInt();
        this.type = tmpType == -1 ? null : NotificationType.values()[tmpType];
        this.typeAsString = in.readString();
        this.objectId = in.readString();
        int tmpStatus = in.readInt();
        this.status = tmpStatus == -1 ? null : NotificationStatus.values()[tmpStatus];
        this.data = in.readString();
        this.numberOfAttempts = (Integer)in.readValue(Integer.class.getClassLoader());
        this.notificationResult = in.readString();
        int tmpObjType = in.readInt();
        this.objType = tmpObjType == -1 ? null : NotificationObjectType.values()[tmpObjType];
    }
}
