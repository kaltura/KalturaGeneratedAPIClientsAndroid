...
public class WidevineRepositorySyncJobData extends JobData {
    ...
    public static final Creator<WidevineRepositorySyncJobData> CREATOR = new Creator<WidevineRepositorySyncJobData>() {
        @Override
        public WidevineRepositorySyncJobData createFromParcel(Parcel source) {
            return new WidevineRepositorySyncJobData(source);
        }

        @Override
        public WidevineRepositorySyncJobData[] newArray(int size) {
            return new WidevineRepositorySyncJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.syncMode == null ? -1 : this.syncMode.ordinal());
        dest.writeString(this.wvAssetIds);
        dest.writeString(this.modifiedAttributes);
        dest.writeValue(this.monitorSyncCompletion);
    }

    public WidevineRepositorySyncJobData(Parcel in) {
        super(in);
        int tmpSyncMode = in.readInt();
        this.syncMode = tmpSyncMode == -1 ? null : WidevineRepositorySyncMode.values()[tmpSyncMode];
        this.wvAssetIds = in.readString();
        this.modifiedAttributes = in.readString();
        this.monitorSyncCompletion = (Integer)in.readValue(Integer.class.getClassLoader());
    }
}
