...
public class UserScorePropertiesFilter extends UserScorePropertiesBaseFilter {
    ...
    public static final Creator<UserScorePropertiesFilter> CREATOR = new Creator<UserScorePropertiesFilter>() {
        @Override
        public UserScorePropertiesFilter createFromParcel(Parcel source) {
            return new UserScorePropertiesFilter(source);
        }

        @Override
        public UserScorePropertiesFilter[] newArray(int size) {
            return new UserScorePropertiesFilter[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.gameObjectId);
        dest.writeInt(this.gameObjectType == null ? -1 : this.gameObjectType.ordinal());
        dest.writeString(this.userIdEqual);
        dest.writeValue(this.placesAboveUser);
        dest.writeValue(this.placesBelowUser);
    }

    public UserScorePropertiesFilter(Parcel in) {
        super(in);
        this.gameObjectId = in.readString();
        int tmpGameObjectType = in.readInt();
        this.gameObjectType = tmpGameObjectType == -1 ? null : GameObjectType.values()[tmpGameObjectType];
        this.userIdEqual = in.readString();
        this.placesAboveUser = (Integer)in.readValue(Integer.class.getClassLoader());
        this.placesBelowUser = (Integer)in.readValue(Integer.class.getClassLoader());
    }
}
