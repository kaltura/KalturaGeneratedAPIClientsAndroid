...
public class AttUverseDistributionJobProviderData extends ConfigurableDistributionJobProviderData {
    ...
    public static final Creator<AttUverseDistributionJobProviderData> CREATOR = new Creator<AttUverseDistributionJobProviderData>() {
        @Override
        public AttUverseDistributionJobProviderData createFromParcel(Parcel source) {
            return new AttUverseDistributionJobProviderData(source);
        }

        @Override
        public AttUverseDistributionJobProviderData[] newArray(int size) {
            return new AttUverseDistributionJobProviderData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.remoteAssetFileUrls);
        dest.writeString(this.remoteThumbnailFileUrls);
        dest.writeString(this.remoteCaptionFileUrls);
    }

    public AttUverseDistributionJobProviderData(Parcel in) {
        super(in);
        int filesForDistributionSize = in.readInt();
        if( filesForDistributionSize > -1) {
            this.filesForDistribution = new ArrayList<>();
            in.readList(this.filesForDistribution, AttUverseDistributionFile.class.getClassLoader());
        }
        this.remoteAssetFileUrls = in.readString();
        this.remoteThumbnailFileUrls = in.readString();
        this.remoteCaptionFileUrls = in.readString();
    }
}
