...
public class AuditTrailChangeInfo extends AuditTrailInfo {
    ...
    public static final Creator<AuditTrailChangeInfo> CREATOR = new Creator<AuditTrailChangeInfo>() {
        @Override
        public AuditTrailChangeInfo createFromParcel(Parcel source) {
            return new AuditTrailChangeInfo(source);
        }

        @Override
        public AuditTrailChangeInfo[] newArray(int size) {
            return new AuditTrailChangeInfo[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.changedItems != null) {
            dest.writeInt(this.changedItems.size());
    ...
    public AuditTrailChangeInfo(Parcel in) {
        super(in);
        int changedItemsSize = in.readInt();
        if( changedItemsSize > -1) {
            this.changedItems = new ArrayList<>();
            in.readList(this.changedItems, AuditTrailChangeItem.class.getClassLoader());
        }
    }
}
