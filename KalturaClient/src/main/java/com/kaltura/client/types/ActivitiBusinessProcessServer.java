...
public class ActivitiBusinessProcessServer extends BusinessProcessServer {
    ...
    public static final Creator<ActivitiBusinessProcessServer> CREATOR = new Creator<ActivitiBusinessProcessServer>() {
        @Override
        public ActivitiBusinessProcessServer createFromParcel(Parcel source) {
            return new ActivitiBusinessProcessServer(source);
        }

        @Override
        public ActivitiBusinessProcessServer[] newArray(int size) {
            return new ActivitiBusinessProcessServer[size];
        }
    };
    ...
    public ActivitiBusinessProcessServer(Parcel in) {
        super(in);
        this.host = in.readString();
        this.port = (Integer)in.readValue(Integer.class.getClassLoader());
        int tmpProtocol = in.readInt();
        this.protocol = tmpProtocol == -1 ? null : ActivitiBusinessProcessServerProtocol.values()[tmpProtocol];
