...
public abstract class UploadTokenBaseFilter extends Filter {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.fileSizeEqual);
    }

    public UploadTokenBaseFilter(Parcel in) {
        super(in);
        this.idEqual = in.readString();
        this.idIn = in.readString();
        this.userIdEqual = in.readString();
        int tmpStatusEqual = in.readInt();
        this.statusEqual = tmpStatusEqual == -1 ? null : UploadTokenStatus.values()[tmpStatusEqual];
        this.statusIn = in.readString();
        this.fileNameEqual = in.readString();
        this.fileSizeEqual = (Double)in.readValue(Double.class.getClassLoader());
    }
}
