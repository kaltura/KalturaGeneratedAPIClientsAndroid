...
public class ComcastMrssDistributionProfile extends ConfigurableDistributionProfile {
    ...
    public static final Creator<ComcastMrssDistributionProfile> CREATOR = new Creator<ComcastMrssDistributionProfile>() {
        @Override
        public ComcastMrssDistributionProfile createFromParcel(Parcel source) {
            return new ComcastMrssDistributionProfile(source);
        }

        @Override
        public ComcastMrssDistributionProfile[] newArray(int size) {
            return new ComcastMrssDistributionProfile[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.feedLastBuildDate);
        dest.writeString(this.itemLink);
        if(this.cPlatformTvSeries != null) {
            dest.writeInt(this.cPlatformTvSeries.size());
            dest.writeList(this.cPlatformTvSeries);
        } else {
            dest.writeInt(-1);
        }
        dest.writeString(this.cPlatformTvSeriesField);
        dest.writeValue(this.shouldIncludeCuePoints);
        dest.writeValue(this.shouldIncludeCaptions);
        dest.writeValue(this.shouldAddThumbExtension);
    }

    public ComcastMrssDistributionProfile(Parcel in) {
        super(in);
        ...
        this.feedLink = in.readString();
        this.feedDescription = in.readString();
        this.feedLastBuildDate = in.readString();
        this.itemLink = in.readString();
        int cPlatformTvSeriesSize = in.readInt();
        if( cPlatformTvSeriesSize > -1) {
            this.cPlatformTvSeries = new ArrayList<>();
            in.readList(this.cPlatformTvSeries, KeyValue.class.getClassLoader());
        }
        this.cPlatformTvSeriesField = in.readString();
        this.shouldIncludeCuePoints = (Boolean)in.readValue(Boolean.class.getClassLoader());
        this.shouldIncludeCaptions = (Boolean)in.readValue(Boolean.class.getClassLoader());
        this.shouldAddThumbExtension = (Boolean)in.readValue(Boolean.class.getClassLoader());
    }
}
