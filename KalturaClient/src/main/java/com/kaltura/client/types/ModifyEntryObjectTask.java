...
public class ModifyEntryObjectTask extends ObjectTask {
    ...
    public static final Creator<ModifyEntryObjectTask> CREATOR = new Creator<ModifyEntryObjectTask>() {
        @Override
        public ModifyEntryObjectTask createFromParcel(Parcel source) {
            return new ModifyEntryObjectTask(source);
        }

        @Override
        public ModifyEntryObjectTask[] newArray(int size) {
            return new ModifyEntryObjectTask[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.inputMetadataProfileId);
        if(this.inputMetadata != null) {
            dest.writeInt(this.inputMetadata.size());
            dest.writeList(this.inputMetadata);
        } else {
            dest.writeInt(-1);
        }
        dest.writeValue(this.outputMetadataProfileId);
        if(this.outputMetadata != null) {
            dest.writeInt(this.outputMetadata.size());
            dest.writeList(this.outputMetadata);
        } else {
            dest.writeInt(-1);
        }
    ...
    public ModifyEntryObjectTask(Parcel in) {
        super(in);
        this.inputMetadataProfileId = (Integer)in.readValue(Integer.class.getClassLoader());
        int inputMetadataSize = in.readInt();
        if( inputMetadataSize > -1) {
            this.inputMetadata = new ArrayList<>();
            in.readList(this.inputMetadata, KeyValue.class.getClassLoader());
        }
        this.outputMetadataProfileId = (Integer)in.readValue(Integer.class.getClassLoader());
        int outputMetadataSize = in.readInt();
        if( outputMetadataSize > -1) {
            this.outputMetadata = new ArrayList<>();
            in.readList(this.outputMetadata, KeyValue.class.getClassLoader());
        }
