...
public class VendorCatalogItemPricing extends ObjectBase {
    ...
    public static final Creator<VendorCatalogItemPricing> CREATOR = new Creator<VendorCatalogItemPricing>() {
        @Override
        public VendorCatalogItemPricing createFromParcel(Parcel source) {
            return new VendorCatalogItemPricing(source);
        }

        @Override
        public VendorCatalogItemPricing[] newArray(int size) {
            return new VendorCatalogItemPricing[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.priceFunction == null ? -1 : this.priceFunction.ordinal());
    }

    public VendorCatalogItemPricing(Parcel in) {
        super(in);
        this.pricePerUnit = (Double)in.readValue(Double.class.getClassLoader());
        int tmpPriceFunction = in.readInt();
        this.priceFunction = tmpPriceFunction == -1 ? null : VendorCatalogItemPriceFunction.values()[tmpPriceFunction];
    }
}
