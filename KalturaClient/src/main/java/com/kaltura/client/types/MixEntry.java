...
public class MixEntry extends PlayableEntry {
    ...
    public static final Creator<MixEntry> CREATOR = new Creator<MixEntry>() {
        @Override
        public MixEntry createFromParcel(Parcel source) {
            return new MixEntry(source);
        }

        @Override
        public MixEntry[] newArray(int size) {
            return new MixEntry[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.hasRealThumbnail);
        dest.writeInt(this.editorType == null ? -1 : this.editorType.ordinal());
        dest.writeString(this.dataContent);
    }

    public MixEntry(Parcel in) {
        super(in);
        this.hasRealThumbnail = (Boolean)in.readValue(Boolean.class.getClassLoader());
        int tmpEditorType = in.readInt();
        this.editorType = tmpEditorType == -1 ? null : EditorType.values()[tmpEditorType];
