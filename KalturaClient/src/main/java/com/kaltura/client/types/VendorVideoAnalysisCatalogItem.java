...
public class VendorVideoAnalysisCatalogItem extends VendorCatalogItem {
    ...
    public static final Creator<VendorVideoAnalysisCatalogItem> CREATOR = new Creator<VendorVideoAnalysisCatalogItem>() {
        @Override
        public VendorVideoAnalysisCatalogItem createFromParcel(Parcel source) {
            return new VendorVideoAnalysisCatalogItem(source);
        }

        @Override
        public VendorVideoAnalysisCatalogItem[] newArray(int size) {
            return new VendorVideoAnalysisCatalogItem[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.videoAnalysisType == null ? -1 : this.videoAnalysisType.ordinal());
        dest.writeValue(this.maxVideoDuration);
    }

    public VendorVideoAnalysisCatalogItem(Parcel in) {
        super(in);
        int tmpVideoAnalysisType = in.readInt();
        this.videoAnalysisType = tmpVideoAnalysisType == -1 ? null : VendorVideoAnalysisType.values()[tmpVideoAnalysisType];
        this.maxVideoDuration = (Integer)in.readValue(Integer.class.getClassLoader());
    }
}
