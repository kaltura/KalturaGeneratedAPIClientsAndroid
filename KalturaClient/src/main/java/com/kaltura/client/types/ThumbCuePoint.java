...
public class ThumbCuePoint extends CuePoint {
    ...
    public static final Creator<ThumbCuePoint> CREATOR = new Creator<ThumbCuePoint>() {
        @Override
        public ThumbCuePoint createFromParcel(Parcel source) {
            return new ThumbCuePoint(source);
        }

        @Override
        public ThumbCuePoint[] newArray(int size) {
            return new ThumbCuePoint[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.subType == null ? -1 : this.subType.ordinal());
    }

    public ThumbCuePoint(Parcel in) {
        super(in);
        this.assetId = in.readString();
        this.description = in.readString();
        this.title = in.readString();
        int tmpSubType = in.readInt();
        this.subType = tmpSubType == -1 ? null : ThumbCuePointSubType.values()[tmpSubType];
    }
}
