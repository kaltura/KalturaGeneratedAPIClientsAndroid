...
public class PlayerDeliveryType extends ObjectBase {
    ...
    public static final Creator<PlayerDeliveryType> CREATOR = new Creator<PlayerDeliveryType>() {
        @Override
        public PlayerDeliveryType createFromParcel(Parcel source) {
            return new PlayerDeliveryType(source);
        }

        @Override
        public PlayerDeliveryType[] newArray(int size) {
            return new PlayerDeliveryType[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.flashvars != null) {
            dest.writeInt(this.flashvars.size());
            dest.writeList(this.flashvars);
        } else {
            dest.writeInt(-1);
        }
        dest.writeString(this.minVersion);
        dest.writeValue(this.enabledByDefault);
    }

    public PlayerDeliveryType(Parcel in) {
        super(in);
        this.id = in.readString();
        this.label = in.readString();
        int flashvarsSize = in.readInt();
        if( flashvarsSize > -1) {
            this.flashvars = new ArrayList<>();
            in.readList(this.flashvars, KeyValue.class.getClassLoader());
        }
        this.minVersion = in.readString();
        this.enabledByDefault = (Boolean)in.readValue(Boolean.class.getClassLoader());
    }
}
