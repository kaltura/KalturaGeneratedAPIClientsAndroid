...
public class CountryRestriction extends BaseRestriction {
    ...
    public static final Creator<CountryRestriction> CREATOR = new Creator<CountryRestriction>() {
        @Override
        public CountryRestriction createFromParcel(Parcel source) {
            return new CountryRestriction(source);
        }

        @Override
        public CountryRestriction[] newArray(int size) {
            return new CountryRestriction[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.countryRestrictionType == null ? -1 : this.countryRestrictionType.ordinal());
        dest.writeString(this.countryList);
    }

    public CountryRestriction(Parcel in) {
        super(in);
        int tmpCountryRestrictionType = in.readInt();
        this.countryRestrictionType = tmpCountryRestrictionType == -1 ? null : CountryRestrictionType.values()[tmpCountryRestrictionType];
        this.countryList = in.readString();
    }
}
