...
public abstract class MediaEntryBaseFilter extends PlayableEntryFilter {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.mediaTypeEqual == null ? -1 : this.mediaTypeEqual.ordinal());
        dest.writeString(this.mediaTypeIn);
        dest.writeInt(this.sourceTypeEqual == null ? -1 : this.sourceTypeEqual.ordinal());
        dest.writeInt(this.sourceTypeNotEqual == null ? -1 : this.sourceTypeNotEqual.ordinal());
        dest.writeString(this.sourceTypeIn);
        dest.writeString(this.sourceTypeNotIn);
        dest.writeValue(this.mediaDateGreaterThanOrEqual);
        dest.writeValue(this.mediaDateLessThanOrEqual);
        dest.writeString(this.flavorParamsIdsMatchOr);
        dest.writeString(this.flavorParamsIdsMatchAnd);
    }

    public MediaEntryBaseFilter(Parcel in) {
        super(in);
        int tmpMediaTypeEqual = in.readInt();
        this.mediaTypeEqual = tmpMediaTypeEqual == -1 ? null : MediaType.values()[tmpMediaTypeEqual];
        this.mediaTypeIn = in.readString();
        int tmpSourceTypeEqual = in.readInt();
        this.sourceTypeEqual = tmpSourceTypeEqual == -1 ? null : SourceType.values()[tmpSourceTypeEqual];
        int tmpSourceTypeNotEqual = in.readInt();
        this.sourceTypeNotEqual = tmpSourceTypeNotEqual == -1 ? null : SourceType.values()[tmpSourceTypeNotEqual];
        this.sourceTypeIn = in.readString();
        this.sourceTypeNotIn = in.readString();
        this.mediaDateGreaterThanOrEqual = (Integer)in.readValue(Integer.class.getClassLoader());
        this.mediaDateLessThanOrEqual = (Integer)in.readValue(Integer.class.getClassLoader());
        this.flavorParamsIdsMatchOr = in.readString();
        this.flavorParamsIdsMatchAnd = in.readString();
    }
}
