...
public class ESearchCategoryEntryItem extends ESearchAbstractEntryItem {
    ...
    public static final Creator<ESearchCategoryEntryItem> CREATOR = new Creator<ESearchCategoryEntryItem>() {
        @Override
        public ESearchCategoryEntryItem createFromParcel(Parcel source) {
            return new ESearchCategoryEntryItem(source);
        }

        @Override
        public ESearchCategoryEntryItem[] newArray(int size) {
            return new ESearchCategoryEntryItem[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.categoryEntryStatus == null ? -1 : this.categoryEntryStatus.ordinal());
    }

    public ESearchCategoryEntryItem(Parcel in) {
        super(in);
        int tmpFieldName = in.readInt();
        this.fieldName = tmpFieldName == -1 ? null : ESearchCategoryEntryFieldName.values()[tmpFieldName];
        int tmpCategoryEntryStatus = in.readInt();
        this.categoryEntryStatus = tmpCategoryEntryStatus == -1 ? null : CategoryEntryStatus.values()[tmpCategoryEntryStatus];
    }
}
