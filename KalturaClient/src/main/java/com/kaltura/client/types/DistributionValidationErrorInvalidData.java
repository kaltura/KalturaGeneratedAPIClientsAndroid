...
public class DistributionValidationErrorInvalidData extends DistributionValidationError {
    ...
    public static final Creator<DistributionValidationErrorInvalidData> CREATOR = new Creator<DistributionValidationErrorInvalidData>() {
        @Override
        public DistributionValidationErrorInvalidData createFromParcel(Parcel source) {
            return new DistributionValidationErrorInvalidData(source);
        }

        @Override
        public DistributionValidationErrorInvalidData[] newArray(int size) {
            return new DistributionValidationErrorInvalidData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.validationErrorType == null ? -1 : this.validationErrorType.ordinal());
        dest.writeString(this.validationErrorParam);
    }

    public DistributionValidationErrorInvalidData(Parcel in) {
        super(in);
        this.fieldName = in.readString();
        int tmpValidationErrorType = in.readInt();
        this.validationErrorType = tmpValidationErrorType == -1 ? null : DistributionValidationErrorType.values()[tmpValidationErrorType];
        this.validationErrorParam = in.readString();
    }
}
