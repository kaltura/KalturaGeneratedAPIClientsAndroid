...
public class ContextDataResult extends ObjectBase {
    ...
    public static final Creator<ContextDataResult> CREATOR = new Creator<ContextDataResult>() {
        @Override
        public ContextDataResult createFromParcel(Parcel source) {
            return new ContextDataResult(source);
        }

        @Override
        public ContextDataResult[] newArray(int size) {
            return new ContextDataResult[size];
        }
    };
    ...
    public ContextDataResult(Parcel in) {
        super(in);
        int messagesSize = in.readInt();
        if( messagesSize > -1) {
            this.messages = new ArrayList<>();
            in.readList(this.messages, StringHolder.class.getClassLoader());
        }
