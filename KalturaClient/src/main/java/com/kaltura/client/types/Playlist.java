...
public class Playlist extends BaseEntry {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.playlistContent);
        ...
        dest.writeString(this.executeUrl);
    }

    public Playlist(Parcel in) {
        super(in);
        this.playlistContent = in.readString();
        int filtersSize = in.readInt();
        if( filtersSize > -1) {
            this.filters = new ArrayList<>();
            in.readList(this.filters, MediaEntryFilterForPlaylist.class.getClassLoader());
        }
        this.totalResults = (Integer)in.readValue(Integer.class.getClassLoader());
        int tmpPlaylistType = in.readInt();
        this.playlistType = tmpPlaylistType == -1 ? null : PlaylistType.values()[tmpPlaylistType];
