...
public class RemoteStorageResources extends ContentResource {
    ...
    public static final Creator<RemoteStorageResources> CREATOR = new Creator<RemoteStorageResources>() {
        @Override
        public RemoteStorageResources createFromParcel(Parcel source) {
            return new RemoteStorageResources(source);
        }

        @Override
        public RemoteStorageResources[] newArray(int size) {
            return new RemoteStorageResources[size];
        }
    };
    ...
    public RemoteStorageResources(Parcel in) {
        super(in);
        int resourcesSize = in.readInt();
        if( resourcesSize > -1) {
            this.resources = new ArrayList<>();
            in.readList(this.resources, RemoteStorageResource.class.getClassLoader());
        }
    }
}
