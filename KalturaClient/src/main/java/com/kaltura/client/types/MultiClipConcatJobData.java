...
public class MultiClipConcatJobData extends JobData {
    ...
    public static final Creator<MultiClipConcatJobData> CREATOR = new Creator<MultiClipConcatJobData>() {
        @Override
        public MultiClipConcatJobData createFromParcel(Parcel source) {
            return new MultiClipConcatJobData(source);
        }

        @Override
        public MultiClipConcatJobData[] newArray(int size) {
            return new MultiClipConcatJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.destEntryId);
        dest.writeString(this.multiTempEntryId);
        dest.writeValue(this.partnerId);
        dest.writeValue(this.priority);
        dest.writeInt(this.chapterNamePolicy == null ? -1 : this.chapterNamePolicy.ordinal());
        dest.writeParcelable(this.aspectRatio, flags);
        if(this.operationResources != null) {
            dest.writeInt(this.operationResources.size());
            dest.writeList(this.operationResources);
        } else {
            dest.writeInt(-1);
        }
    }

    public MultiClipConcatJobData(Parcel in) {
        super(in);
        this.destEntryId = in.readString();
        this.multiTempEntryId = in.readString();
        ...
        this.aspectRatio = in.readParcelable(CropAspectRatio.class.getClassLoader());
        int operationResourcesSize = in.readInt();
        if( operationResourcesSize > -1) {
            this.operationResources = new ArrayList<>();
            in.readList(this.operationResources, OperationResource.class.getClassLoader());
        }
    }
}
