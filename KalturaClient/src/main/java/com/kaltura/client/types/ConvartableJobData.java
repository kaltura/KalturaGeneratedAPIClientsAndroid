...
public class ConvartableJobData extends JobData {
    ...
    public static final Creator<ConvartableJobData> CREATOR = new Creator<ConvartableJobData>() {
        @Override
        public ConvartableJobData createFromParcel(Parcel source) {
            return new ConvartableJobData(source);
        }

        @Override
        public ConvartableJobData[] newArray(int size) {
            return new ConvartableJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.srcFileSyncs != null) {
            dest.writeInt(this.srcFileSyncs.size());
            dest.writeList(this.srcFileSyncs);
        } else {
            dest.writeInt(-1);
        }
        dest.writeValue(this.engineVersion);
        dest.writeValue(this.flavorParamsOutputId);
        dest.writeParcelable(this.flavorParamsOutput, flags);
        dest.writeValue(this.mediaInfoId);
        dest.writeValue(this.currentOperationSet);
        dest.writeValue(this.currentOperationIndex);
        if(this.pluginData != null) {
            dest.writeInt(this.pluginData.size());
            dest.writeList(this.pluginData);
        } else {
            dest.writeInt(-1);
        }
    }

    public ConvartableJobData(Parcel in) {
        super(in);
        ...
        int srcFileSyncsSize = in.readInt();
        if( srcFileSyncsSize > -1) {
            this.srcFileSyncs = new ArrayList<>();
            in.readList(this.srcFileSyncs, SourceFileSyncDescriptor.class.getClassLoader());
        }
        this.engineVersion = (Integer)in.readValue(Integer.class.getClassLoader());
        this.flavorParamsOutputId = (Integer)in.readValue(Integer.class.getClassLoader());
        this.flavorParamsOutput = in.readParcelable(FlavorParamsOutput.class.getClassLoader());
        this.mediaInfoId = (Integer)in.readValue(Integer.class.getClassLoader());
        this.currentOperationSet = (Integer)in.readValue(Integer.class.getClassLoader());
        this.currentOperationIndex = (Integer)in.readValue(Integer.class.getClassLoader());
        int pluginDataSize = in.readInt();
        if( pluginDataSize > -1) {
            this.pluginData = new ArrayList<>();
            in.readList(this.pluginData, KeyValue.class.getClassLoader());
        }
    }
}
