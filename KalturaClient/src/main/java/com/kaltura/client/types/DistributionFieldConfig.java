...
public class DistributionFieldConfig extends ObjectBase {
    ...
    public static final Creator<DistributionFieldConfig> CREATOR = new Creator<DistributionFieldConfig>() {
        @Override
        public DistributionFieldConfig createFromParcel(Parcel source) {
            return new DistributionFieldConfig(source);
        }

        @Override
        public DistributionFieldConfig[] newArray(int size) {
            return new DistributionFieldConfig[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.updateParams != null) {
            ...
            dest.writeList(this.updateParams);
        } else {
            dest.writeInt(-1);
        }
        dest.writeValue(this.isDefault);
        dest.writeValue(this.triggerDeleteOnError);
    }

    public DistributionFieldConfig(Parcel in) {
        super(in);
        this.fieldName = in.readString();
        this.userFriendlyFieldName = in.readString();
        ...
        if( updateParamsSize > -1) {
            ...
            in.readList(this.updateParams, StringHolder.class.getClassLoader());
        }
        this.isDefault = (Boolean)in.readValue(Boolean.class.getClassLoader());
        this.triggerDeleteOnError = (Boolean)in.readValue(Boolean.class.getClassLoader());
    }
}
