...
public class DrmProfile extends ObjectBase {
    ...
    public static final Creator<DrmProfile> CREATOR = new Creator<DrmProfile>() {
        @Override
        public DrmProfile createFromParcel(Parcel source) {
            return new DrmProfile(source);
        }

        @Override
        public DrmProfile[] newArray(int size) {
            return new DrmProfile[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.defaultPolicy);
        dest.writeValue(this.createdAt);
        dest.writeValue(this.updatedAt);
        dest.writeString(this.signingKey);
    }

    public DrmProfile(Parcel in) {
        super(in);
        ...
        this.status = tmpStatus == -1 ? null : DrmProfileStatus.values()[tmpStatus];
        this.licenseServerUrl = in.readString();
        this.defaultPolicy = in.readString();
        this.createdAt = (Integer)in.readValue(Integer.class.getClassLoader());
        this.updatedAt = (Integer)in.readValue(Integer.class.getClassLoader());
        this.signingKey = in.readString();
    }
}
