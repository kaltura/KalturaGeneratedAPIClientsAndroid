...
public class ConcatJobData extends JobData {
    ...
    public static final Creator<ConcatJobData> CREATOR = new Creator<ConcatJobData>() {
        @Override
        public ConcatJobData createFromParcel(Parcel source) {
            return new ConcatJobData(source);
        }

        @Override
        public ConcatJobData[] newArray(int size) {
            return new ConcatJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.srcFiles != null) {
            dest.writeInt(this.srcFiles.size());
            dest.writeList(this.srcFiles);
        } else {
            dest.writeInt(-1);
        }
