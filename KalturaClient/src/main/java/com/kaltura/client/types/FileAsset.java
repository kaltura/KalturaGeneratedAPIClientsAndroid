...
public class FileAsset extends ObjectBase {
    ...
    public static final Creator<FileAsset> CREATOR = new Creator<FileAsset>() {
        @Override
        public FileAsset createFromParcel(Parcel source) {
            return new FileAsset(source);
        }

        @Override
        public FileAsset[] newArray(int size) {
            return new FileAsset[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.fileAssetObjectType == null ? -1 : this.fileAssetObjectType.ordinal());
    ...
    public FileAsset(Parcel in) {
        super(in);
        this.id = (Long)in.readValue(Long.class.getClassLoader());
        this.partnerId = (Integer)in.readValue(Integer.class.getClassLoader());
        int tmpFileAssetObjectType = in.readInt();
        this.fileAssetObjectType = tmpFileAssetObjectType == -1 ? null : FileAssetObjectType.values()[tmpFileAssetObjectType];
        ...
        this.status = tmpStatus == -1 ? null : FileAssetStatus.values()[tmpStatus];
    }
}
