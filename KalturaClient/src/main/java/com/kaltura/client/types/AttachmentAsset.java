...
public class AttachmentAsset extends Asset {
    ...
    public static final Creator<AttachmentAsset> CREATOR = new Creator<AttachmentAsset>() {
        @Override
        public AttachmentAsset createFromParcel(Parcel source) {
            return new AttachmentAsset(source);
        }

        @Override
        public AttachmentAsset[] newArray(int size) {
            return new AttachmentAsset[size];
        }
    };
    ...
    public AttachmentAsset(Parcel in) {
        super(in);
        this.filename = in.readString();
        this.title = in.readString();
        int tmpFormat = in.readInt();
        this.format = tmpFormat == -1 ? null : AttachmentType.values()[tmpFormat];
        int tmpStatus = in.readInt();
        this.status = tmpStatus == -1 ? null : AttachmentAssetStatus.values()[tmpStatus];
    }
}
