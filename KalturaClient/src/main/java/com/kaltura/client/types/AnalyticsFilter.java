...
public class AnalyticsFilter extends ObjectBase {
    ...
    public static final Creator<AnalyticsFilter> CREATOR = new Creator<AnalyticsFilter>() {
        @Override
        public AnalyticsFilter createFromParcel(Parcel source) {
            return new AnalyticsFilter(source);
        }

        @Override
        public AnalyticsFilter[] newArray(int size) {
            return new AnalyticsFilter[size];
        }
    };
    ...
    public AnalyticsFilter(Parcel in) {
        super(in);
        this.from_time = in.readString();
        this.to_time = in.readString();
        this.metrics = in.readString();
        this.utcOffset = (Double)in.readValue(Double.class.getClassLoader());
        this.dimensions = in.readString();
        int filtersSize = in.readInt();
        if( filtersSize > -1) {
            this.filters = new ArrayList<>();
            in.readList(this.filters, ReportFilter.class.getClassLoader());
        }
