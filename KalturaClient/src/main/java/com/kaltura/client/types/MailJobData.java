...
public class MailJobData extends JobData {
    ...
    public static final Creator<MailJobData> CREATOR = new Creator<MailJobData>() {
        @Override
        public MailJobData createFromParcel(Parcel source) {
            return new MailJobData(source);
        }

        @Override
        public MailJobData[] newArray(int size) {
            return new MailJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.mailType == null ? -1 : this.mailType.ordinal());
        dest.writeValue(this.mailPriority);
        ...
        dest.writeValue(this.minSendDate);
        dest.writeValue(this.isHtml);
        dest.writeString(this.separator);
    }

    public MailJobData(Parcel in) {
        super(in);
        int tmpMailType = in.readInt();
        this.mailType = tmpMailType == -1 ? null : MailType.values()[tmpMailType];
        this.mailPriority = (Integer)in.readValue(Integer.class.getClassLoader());
        int tmpStatus = in.readInt();
        this.status = tmpStatus == -1 ? null : MailJobStatus.values()[tmpStatus];
        ...
        this.minSendDate = (Integer)in.readValue(Integer.class.getClassLoader());
        this.isHtml = (Boolean)in.readValue(Boolean.class.getClassLoader());
