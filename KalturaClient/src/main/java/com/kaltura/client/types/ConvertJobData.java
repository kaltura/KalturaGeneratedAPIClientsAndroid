...
public class ConvertJobData extends ConvartableJobData {
    ...
    public static final Creator<ConvertJobData> CREATOR = new Creator<ConvertJobData>() {
        @Override
        public ConvertJobData createFromParcel(Parcel source) {
            return new ConvertJobData(source);
        }

        @Override
        public ConvertJobData[] newArray(int size) {
            return new ConvertJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.logFileSyncLocalPath);
        dest.writeString(this.logFileSyncRemoteUrl);
        dest.writeString(this.flavorAssetId);
        dest.writeString(this.remoteMediaId);
        dest.writeString(this.customData);
        if(this.extraDestFileSyncs != null) {
            dest.writeInt(this.extraDestFileSyncs.size());
            dest.writeList(this.extraDestFileSyncs);
        } else {
            dest.writeInt(-1);
        }
    ...
    public ConvertJobData(Parcel in) {
        ...
        this.logFileSyncLocalPath = in.readString();
        this.logFileSyncRemoteUrl = in.readString();
        this.flavorAssetId = in.readString();
        this.remoteMediaId = in.readString();
        this.customData = in.readString();
        int extraDestFileSyncsSize = in.readInt();
        if( extraDestFileSyncsSize > -1) {
            this.extraDestFileSyncs = new ArrayList<>();
