...
public class ConvertCollectionJobData extends ConvartableJobData {
    ...
    public static final Creator<ConvertCollectionJobData> CREATOR = new Creator<ConvertCollectionJobData>() {
        @Override
        public ConvertCollectionJobData createFromParcel(Parcel source) {
            return new ConvertCollectionJobData(source);
        }

        @Override
        public ConvertCollectionJobData[] newArray(int size) {
            return new ConvertCollectionJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.destDirLocalPath);
        dest.writeString(this.destDirRemoteUrl);
        dest.writeString(this.destFileName);
        dest.writeString(this.inputXmlLocalPath);
        dest.writeString(this.inputXmlRemoteUrl);
        dest.writeString(this.commandLinesStr);
        if(this.flavors != null) {
            dest.writeInt(this.flavors.size());
            dest.writeList(this.flavors);
        } else {
            dest.writeInt(-1);
        }
    }

    public ConvertCollectionJobData(Parcel in) {
        super(in);
        this.destDirLocalPath = in.readString();
        this.destDirRemoteUrl = in.readString();
        this.destFileName = in.readString();
        this.inputXmlLocalPath = in.readString();
        this.inputXmlRemoteUrl = in.readString();
        this.commandLinesStr = in.readString();
        int flavorsSize = in.readInt();
        if( flavorsSize > -1) {
            this.flavors = new ArrayList<>();
            in.readList(this.flavors, ConvertCollectionFlavorData.class.getClassLoader());
        }
    }
}
