...
public class VirtualScheduleEvent extends ScheduleEvent {
    ...
    public static final Creator<VirtualScheduleEvent> CREATOR = new Creator<VirtualScheduleEvent>() {
        @Override
        public VirtualScheduleEvent createFromParcel(Parcel source) {
            return new VirtualScheduleEvent(source);
        }

        @Override
        public VirtualScheduleEvent[] newArray(int size) {
            return new VirtualScheduleEvent[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.virtualEventId);
        dest.writeInt(this.virtualScheduleEventSubType == null ? -1 : this.virtualScheduleEventSubType.ordinal());
    }

    public VirtualScheduleEvent(Parcel in) {
        super(in);
        this.virtualEventId = (Integer)in.readValue(Integer.class.getClassLoader());
        int tmpVirtualScheduleEventSubType = in.readInt();
        this.virtualScheduleEventSubType = tmpVirtualScheduleEventSubType == -1 ? null : VirtualScheduleEventSubType.values()[tmpVirtualScheduleEventSubType];
    }
}
