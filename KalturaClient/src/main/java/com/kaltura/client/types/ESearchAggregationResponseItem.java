...
public class ESearchAggregationResponseItem extends ObjectBase {
    ...
    public static final Creator<ESearchAggregationResponseItem> CREATOR = new Creator<ESearchAggregationResponseItem>() {
        @Override
        public ESearchAggregationResponseItem createFromParcel(Parcel source) {
            return new ESearchAggregationResponseItem(source);
        }

        @Override
        public ESearchAggregationResponseItem[] newArray(int size) {
            return new ESearchAggregationResponseItem[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.buckets != null) {
            dest.writeInt(this.buckets.size());
            dest.writeList(this.buckets);
        } else {
            dest.writeInt(-1);
        }
    }

    public ESearchAggregationResponseItem(Parcel in) {
        super(in);
        ...
        if( bucketsSize > -1) {
            ...
            in.readList(this.buckets, ESearchAggregationBucket.class.getClassLoader());
        }
    }
}
