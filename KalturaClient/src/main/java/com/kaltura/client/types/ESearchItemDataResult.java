...
public class ESearchItemDataResult extends ObjectBase {
    ...
    public static final Creator<ESearchItemDataResult> CREATOR = new Creator<ESearchItemDataResult>() {
        @Override
        public ESearchItemDataResult createFromParcel(Parcel source) {
            return new ESearchItemDataResult(source);
        }

        @Override
        public ESearchItemDataResult[] newArray(int size) {
            return new ESearchItemDataResult[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.itemsType);
    }

    public ESearchItemDataResult(Parcel in) {
        super(in);
        this.totalCount = (Integer)in.readValue(Integer.class.getClassLoader());
        int itemsSize = in.readInt();
        if( itemsSize > -1) {
            this.items = new ArrayList<>();
            in.readList(this.items, ESearchItemData.class.getClassLoader());
        }
        this.itemsType = in.readString();
    }
}
