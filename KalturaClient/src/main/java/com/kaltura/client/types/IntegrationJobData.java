...
public class IntegrationJobData extends JobData {
    ...
    public static final Creator<IntegrationJobData> CREATOR = new Creator<IntegrationJobData>() {
        @Override
        public IntegrationJobData createFromParcel(Parcel source) {
            return new IntegrationJobData(source);
        }

        @Override
        public IntegrationJobData[] newArray(int size) {
            return new IntegrationJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.callbackNotificationUrl);
        dest.writeInt(this.providerType == null ? -1 : this.providerType.ordinal());
        dest.writeParcelable(this.providerData, flags);
        dest.writeInt(this.triggerType == null ? -1 : this.triggerType.ordinal());
        dest.writeParcelable(this.triggerData, flags);
    }

    public IntegrationJobData(Parcel in) {
        super(in);
        this.callbackNotificationUrl = in.readString();
        int tmpProviderType = in.readInt();
        this.providerType = tmpProviderType == -1 ? null : IntegrationProviderType.values()[tmpProviderType];
        this.providerData = in.readParcelable(IntegrationJobProviderData.class.getClassLoader());
        int tmpTriggerType = in.readInt();
        this.triggerType = tmpTriggerType == -1 ? null : IntegrationTriggerType.values()[tmpTriggerType];
        this.triggerData = in.readParcelable(IntegrationJobTriggerData.class.getClassLoader());
    }
}
