...
public class EmailNotificationStaticRecipientProvider extends EmailNotificationRecipientProvider {
    ...
    public static final Creator<EmailNotificationStaticRecipientProvider> CREATOR = new Creator<EmailNotificationStaticRecipientProvider>() {
        @Override
        public EmailNotificationStaticRecipientProvider createFromParcel(Parcel source) {
            return new EmailNotificationStaticRecipientProvider(source);
        }

        @Override
        public EmailNotificationStaticRecipientProvider[] newArray(int size) {
            return new EmailNotificationStaticRecipientProvider[size];
        }
    };
    ...
    public EmailNotificationStaticRecipientProvider(Parcel in) {
        super(in);
        int emailRecipientsSize = in.readInt();
        if( emailRecipientsSize > -1) {
            this.emailRecipients = new ArrayList<>();
            in.readList(this.emailRecipients, EmailNotificationRecipient.class.getClassLoader());
        }
    }
}
