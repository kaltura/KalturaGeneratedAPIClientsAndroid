...
public class DrmPolicy extends ObjectBase {
    ...
    public static final Creator<DrmPolicy> CREATOR = new Creator<DrmPolicy>() {
        @Override
        public DrmPolicy createFromParcel(Parcel source) {
            return new DrmPolicy(source);
        }

        @Override
        public DrmPolicy[] newArray(int size) {
            return new DrmPolicy[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.scenario == null ? -1 : this.scenario.ordinal());
        dest.writeInt(this.licenseType == null ? -1 : this.licenseType.ordinal());
        dest.writeInt(this.licenseExpirationPolicy == null ? -1 : this.licenseExpirationPolicy.ordinal());
        dest.writeValue(this.duration);
        dest.writeValue(this.createdAt);
        dest.writeValue(this.updatedAt);
        if(this.licenseParams != null) {
            dest.writeInt(this.licenseParams.size());
            dest.writeList(this.licenseParams);
        } else {
            dest.writeInt(-1);
        }
    }

    public DrmPolicy(Parcel in) {
        super(in);
        ...
        this.provider = tmpProvider == -1 ? null : DrmProviderType.values()[tmpProvider];
        int tmpStatus = in.readInt();
        this.status = tmpStatus == -1 ? null : DrmPolicyStatus.values()[tmpStatus];
        int tmpScenario = in.readInt();
        this.scenario = tmpScenario == -1 ? null : DrmLicenseScenario.values()[tmpScenario];
        int tmpLicenseType = in.readInt();
        this.licenseType = tmpLicenseType == -1 ? null : DrmLicenseType.values()[tmpLicenseType];
        int tmpLicenseExpirationPolicy = in.readInt();
        this.licenseExpirationPolicy = tmpLicenseExpirationPolicy == -1 ? null : DrmLicenseExpirationPolicy.values()[tmpLicenseExpirationPolicy];
        this.duration = (Integer)in.readValue(Integer.class.getClassLoader());
        this.createdAt = (Integer)in.readValue(Integer.class.getClassLoader());
        this.updatedAt = (Integer)in.readValue(Integer.class.getClassLoader());
        int licenseParamsSize = in.readInt();
        if( licenseParamsSize > -1) {
            this.licenseParams = new ArrayList<>();
            in.readList(this.licenseParams, KeyValue.class.getClassLoader());
        }
    }
}
