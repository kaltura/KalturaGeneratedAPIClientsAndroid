...
public class ESearchGroupResponse extends ESearchResponse {
    ...
    public static final Creator<ESearchGroupResponse> CREATOR = new Creator<ESearchGroupResponse>() {
        @Override
        public ESearchGroupResponse createFromParcel(Parcel source) {
            return new ESearchGroupResponse(source);
        }

        @Override
        public ESearchGroupResponse[] newArray(int size) {
            return new ESearchGroupResponse[size];
        }
    };
    ...
    public ESearchGroupResponse(Parcel in) {
        super(in);
        int objectsSize = in.readInt();
        if( objectsSize > -1) {
            this.objects = new ArrayList<>();
            in.readList(this.objects, ESearchGroupResult.class.getClassLoader());
        }
    }
}
