...
public class EventNotificationScope extends Scope {
    ...
    public static final Creator<EventNotificationScope> CREATOR = new Creator<EventNotificationScope>() {
        @Override
        public EventNotificationScope createFromParcel(Parcel source) {
            return new EventNotificationScope(source);
        }

        @Override
        public EventNotificationScope[] newArray(int size) {
            return new EventNotificationScope[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.scopeObjectType == null ? -1 : this.scopeObjectType.ordinal());
    }

    public EventNotificationScope(Parcel in) {
        super(in);
        this.objectId = in.readString();
        int tmpScopeObjectType = in.readInt();
        this.scopeObjectType = tmpScopeObjectType == -1 ? null : EventNotificationEventObjectType.values()[tmpScopeObjectType];
    }
}
