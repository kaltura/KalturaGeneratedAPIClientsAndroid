...
public class GoogleVideoSyndicationFeed extends BaseSyndicationFeed {
    ...
    public static final Creator<GoogleVideoSyndicationFeed> CREATOR = new Creator<GoogleVideoSyndicationFeed>() {
        @Override
        public GoogleVideoSyndicationFeed createFromParcel(Parcel source) {
            return new GoogleVideoSyndicationFeed(source);
        }

        @Override
        public GoogleVideoSyndicationFeed[] newArray(int size) {
            return new GoogleVideoSyndicationFeed[size];
        }
    };
    ...
    public GoogleVideoSyndicationFeed(Parcel in) {
        super(in);
        int tmpAdultContent = in.readInt();
        this.adultContent = tmpAdultContent == -1 ? null : GoogleSyndicationFeedAdultValues.values()[tmpAdultContent];
    }
}
