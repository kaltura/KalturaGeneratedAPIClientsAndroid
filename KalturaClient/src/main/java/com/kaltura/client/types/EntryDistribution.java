...
public class EntryDistribution extends ObjectBase {
    ...
    public static final Creator<EntryDistribution> CREATOR = new Creator<EntryDistribution>() {
        @Override
        public EntryDistribution createFromParcel(Parcel source) {
            return new EntryDistribution(source);
        }

        @Override
        public EntryDistribution[] newArray(int size) {
            return new EntryDistribution[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.submittedAt);
        dest.writeString(this.entryId);
        dest.writeValue(this.partnerId);
        dest.writeValue(this.distributionProfileId);
        dest.writeInt(this.status == null ? -1 : this.status.ordinal());
        dest.writeInt(this.sunStatus == null ? -1 : this.sunStatus.ordinal());
        dest.writeInt(this.dirtyStatus == null ? -1 : this.dirtyStatus.ordinal());
        dest.writeString(this.thumbAssetIds);
        dest.writeString(this.flavorAssetIds);
        ...
        if(this.validationErrors != null) {
            dest.writeInt(this.validationErrors.size());
            dest.writeList(this.validationErrors);
        } else {
            dest.writeInt(-1);
        }
    ...
    public EntryDistribution(Parcel in) {
        ...
        this.status = tmpStatus == -1 ? null : EntryDistributionStatus.values()[tmpStatus];
        int tmpSunStatus = in.readInt();
        this.sunStatus = tmpSunStatus == -1 ? null : EntryDistributionSunStatus.values()[tmpSunStatus];
        int tmpDirtyStatus = in.readInt();
        this.dirtyStatus = tmpDirtyStatus == -1 ? null : EntryDistributionFlag.values()[tmpDirtyStatus];
        ...
        int validationErrorsSize = in.readInt();
        if( validationErrorsSize > -1) {
            this.validationErrors = new ArrayList<>();
            in.readList(this.validationErrors, DistributionValidationError.class.getClassLoader());
        }
