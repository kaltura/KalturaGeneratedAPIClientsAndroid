...
public class LiveChannelSegment extends ObjectBase {
    ...
    public static final Creator<LiveChannelSegment> CREATOR = new Creator<LiveChannelSegment>() {
        @Override
        public LiveChannelSegment createFromParcel(Parcel source) {
            return new LiveChannelSegment(source);
        }

        @Override
        public LiveChannelSegment[] newArray(int size) {
            return new LiveChannelSegment[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.triggerSegmentId);
        dest.writeValue(this.startTime);
        dest.writeValue(this.duration);
    }

    public LiveChannelSegment(Parcel in) {
        super(in);
        ...
        this.type = tmpType == -1 ? null : LiveChannelSegmentType.values()[tmpType];
        int tmpStatus = in.readInt();
        this.status = tmpStatus == -1 ? null : LiveChannelSegmentStatus.values()[tmpStatus];
        this.channelId = in.readString();
        this.entryId = in.readString();
        int tmpTriggerType = in.readInt();
        this.triggerType = tmpTriggerType == -1 ? null : LiveChannelSegmentTriggerType.values()[tmpTriggerType];
        this.triggerSegmentId = (Long)in.readValue(Long.class.getClassLoader());
        this.startTime = (Double)in.readValue(Double.class.getClassLoader());
