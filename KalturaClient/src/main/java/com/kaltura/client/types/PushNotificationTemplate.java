...
public class PushNotificationTemplate extends EventNotificationTemplate {
    ...
    public static final Creator<PushNotificationTemplate> CREATOR = new Creator<PushNotificationTemplate>() {
        @Override
        public PushNotificationTemplate createFromParcel(Parcel source) {
            return new PushNotificationTemplate(source);
        }

        @Override
        public PushNotificationTemplate[] newArray(int size) {
            return new PushNotificationTemplate[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.queueNameParameters != null) {
            dest.writeInt(this.queueNameParameters.size());
            dest.writeList(this.queueNameParameters);
        } else {
            dest.writeInt(-1);
        }
        if(this.queueKeyParameters != null) {
            dest.writeInt(this.queueKeyParameters.size());
            dest.writeList(this.queueKeyParameters);
        } else {
            dest.writeInt(-1);
        }
        dest.writeString(this.apiObjectType);
        dest.writeInt(this.objectFormat == null ? -1 : this.objectFormat.ordinal());
        dest.writeValue(this.responseProfileId);
    }

    public PushNotificationTemplate(Parcel in) {
        super(in);
        int queueNameParametersSize = in.readInt();
        if( queueNameParametersSize > -1) {
            this.queueNameParameters = new ArrayList<>();
            in.readList(this.queueNameParameters, PushEventNotificationParameter.class.getClassLoader());
        }
        int queueKeyParametersSize = in.readInt();
        if( queueKeyParametersSize > -1) {
            this.queueKeyParameters = new ArrayList<>();
            in.readList(this.queueKeyParameters, PushEventNotificationParameter.class.getClassLoader());
        }
        this.apiObjectType = in.readString();
        int tmpObjectFormat = in.readInt();
        this.objectFormat = tmpObjectFormat == -1 ? null : ResponseType.values()[tmpObjectFormat];
