...
public abstract class MediaServerNode extends DeliveryServerNode {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.mediaServerPortConfig != null) {
            dest.writeInt(this.mediaServerPortConfig.size());
            dest.writeList(this.mediaServerPortConfig);
        } else {
            dest.writeInt(-1);
        }
        if(this.mediaServerPlaybackDomainConfig != null) {
            dest.writeInt(this.mediaServerPlaybackDomainConfig.size());
            dest.writeList(this.mediaServerPlaybackDomainConfig);
        } else {
            dest.writeInt(-1);
        }
    }

    public MediaServerNode(Parcel in) {
        super(in);
        this.applicationName = in.readString();
        int mediaServerPortConfigSize = in.readInt();
        if( mediaServerPortConfigSize > -1) {
            this.mediaServerPortConfig = new ArrayList<>();
            in.readList(this.mediaServerPortConfig, KeyValue.class.getClassLoader());
        }
        int mediaServerPlaybackDomainConfigSize = in.readInt();
        if( mediaServerPlaybackDomainConfigSize > -1) {
            this.mediaServerPlaybackDomainConfig = new ArrayList<>();
            in.readList(this.mediaServerPlaybackDomainConfig, KeyValue.class.getClassLoader());
        }
    }
}
