...
public class AkamaiUniversalProvisionJobData extends ProvisionJobData {
    ...
    public static final Creator<AkamaiUniversalProvisionJobData> CREATOR = new Creator<AkamaiUniversalProvisionJobData>() {
        @Override
        public AkamaiUniversalProvisionJobData createFromParcel(Parcel source) {
            return new AkamaiUniversalProvisionJobData(source);
        }

        @Override
        public AkamaiUniversalProvisionJobData[] newArray(int size) {
            return new AkamaiUniversalProvisionJobData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.streamId);
        dest.writeString(this.systemUserName);
        dest.writeString(this.systemPassword);
        dest.writeString(this.domainName);
        dest.writeInt(this.dvrEnabled == null ? -1 : this.dvrEnabled.ordinal());
        dest.writeValue(this.dvrWindow);
        dest.writeString(this.primaryContact);
        dest.writeString(this.secondaryContact);
        dest.writeInt(this.streamType == null ? -1 : this.streamType.ordinal());
        dest.writeString(this.notificationEmail);
    }

    public AkamaiUniversalProvisionJobData(Parcel in) {
        super(in);
        this.streamId = (Integer)in.readValue(Integer.class.getClassLoader());
        this.systemUserName = in.readString();
        this.systemPassword = in.readString();
        this.domainName = in.readString();
        int tmpDvrEnabled = in.readInt();
        this.dvrEnabled = tmpDvrEnabled == -1 ? null : DVRStatus.values()[tmpDvrEnabled];
        this.dvrWindow = (Integer)in.readValue(Integer.class.getClassLoader());
        this.primaryContact = in.readString();
        this.secondaryContact = in.readString();
        int tmpStreamType = in.readInt();
        this.streamType = tmpStreamType == -1 ? null : AkamaiUniversalStreamType.values()[tmpStreamType];
