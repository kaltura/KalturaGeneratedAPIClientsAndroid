...
public class Sso extends ObjectBase {
    ...
    public static final Creator<Sso> CREATOR = new Creator<Sso>() {
        @Override
        public Sso createFromParcel(Parcel source) {
            return new Sso(source);
        }

        @Override
        public Sso[] newArray(int size) {
            return new Sso[size];
        }
    };
    ...
    public Sso(Parcel in) {
        ...
        this.status = tmpStatus == -1 ? null : SsoStatus.values()[tmpStatus];
