...
public class ZoomRecordingFile extends ObjectBase {
    ...
    public static final Creator<ZoomRecordingFile> CREATOR = new Creator<ZoomRecordingFile>() {
        @Override
        public ZoomRecordingFile createFromParcel(Parcel source) {
            return new ZoomRecordingFile(source);
        }

        @Override
        public ZoomRecordingFile[] newArray(int size) {
            return new ZoomRecordingFile[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.recordingStart);
        dest.writeInt(this.fileType == null ? -1 : this.fileType.ordinal());
        dest.writeString(this.downloadUrl);
        dest.writeString(this.fileExtension);
        dest.writeString(this.downloadToken);
    }

    public ZoomRecordingFile(Parcel in) {
        super(in);
        this.id = in.readString();
        this.recordingStart = in.readString();
        int tmpFileType = in.readInt();
        this.fileType = tmpFileType == -1 ? null : RecordingFileType.values()[tmpFileType];
        this.downloadUrl = in.readString();
        this.fileExtension = in.readString();
        this.downloadToken = in.readString();
    }
}
