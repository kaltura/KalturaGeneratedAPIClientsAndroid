...
public class CategoryEntryAdvancedFilter extends SearchItem {
    ...
    public static final Creator<CategoryEntryAdvancedFilter> CREATOR = new Creator<CategoryEntryAdvancedFilter>() {
        @Override
        public CategoryEntryAdvancedFilter createFromParcel(Parcel source) {
            return new CategoryEntryAdvancedFilter(source);
        }

        @Override
        public CategoryEntryAdvancedFilter[] newArray(int size) {
            return new CategoryEntryAdvancedFilter[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.categoryEntryStatusIn);
        dest.writeInt(this.orderBy == null ? -1 : this.orderBy.ordinal());
        dest.writeValue(this.categoryIdEqual);
    }

    public CategoryEntryAdvancedFilter(Parcel in) {
        super(in);
        this.categoriesMatchOr = in.readString();
        this.categoryEntryStatusIn = in.readString();
        int tmpOrderBy = in.readInt();
        this.orderBy = tmpOrderBy == -1 ? null : CategoryEntryAdvancedOrderBy.values()[tmpOrderBy];
