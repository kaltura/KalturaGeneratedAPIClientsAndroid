...
public class AuditTrailFileSyncCreateInfo extends AuditTrailInfo {
    ...
    public static final Creator<AuditTrailFileSyncCreateInfo> CREATOR = new Creator<AuditTrailFileSyncCreateInfo>() {
        @Override
        public AuditTrailFileSyncCreateInfo createFromParcel(Parcel source) {
            return new AuditTrailFileSyncCreateInfo(source);
        }

        @Override
        public AuditTrailFileSyncCreateInfo[] newArray(int size) {
            return new AuditTrailFileSyncCreateInfo[size];
        }
    };
    ...
    public AuditTrailFileSyncCreateInfo(Parcel in) {
        super(in);
        ...
        this.original = (Boolean)in.readValue(Boolean.class.getClassLoader());
        int tmpFileType = in.readInt();
        this.fileType = tmpFileType == -1 ? null : AuditTrailFileSyncType.values()[tmpFileType];
    }
}
