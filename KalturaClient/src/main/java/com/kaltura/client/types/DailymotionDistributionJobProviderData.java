...
public class DailymotionDistributionJobProviderData extends ConfigurableDistributionJobProviderData {
    ...
    public static final Creator<DailymotionDistributionJobProviderData> CREATOR = new Creator<DailymotionDistributionJobProviderData>() {
        @Override
        public DailymotionDistributionJobProviderData createFromParcel(Parcel source) {
            return new DailymotionDistributionJobProviderData(source);
        }

        @Override
        public DailymotionDistributionJobProviderData[] newArray(int size) {
            return new DailymotionDistributionJobProviderData[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.accessControlGeoBlockingOperation);
        dest.writeString(this.accessControlGeoBlockingCountryList);
    ...
    public DailymotionDistributionJobProviderData(Parcel in) {
        super(in);
        this.videoAssetFilePath = in.readString();
        this.accessControlGeoBlockingOperation = in.readString();
        this.accessControlGeoBlockingCountryList = in.readString();
        int captionsInfoSize = in.readInt();
        if( captionsInfoSize > -1) {
            this.captionsInfo = new ArrayList<>();
            in.readList(this.captionsInfo, DailymotionDistributionCaptionInfo.class.getClassLoader());
        }
    }
}
