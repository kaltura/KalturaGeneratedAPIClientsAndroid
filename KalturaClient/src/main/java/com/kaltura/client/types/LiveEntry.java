...
public abstract class LiveEntry extends MediaEntry {
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeString(this.offlineMessage);
        dest.writeInt(this.recordStatus == null ? -1 : this.recordStatus.ordinal());
        dest.writeInt(this.dvrStatus == null ? -1 : this.dvrStatus.ordinal());
        dest.writeValue(this.dvrWindow);
        dest.writeValue(this.lastElapsedRecordingTime);
        if(this.liveStreamConfigurations != null) {
            dest.writeInt(this.liveStreamConfigurations.size());
            dest.writeList(this.liveStreamConfigurations);
        } else {
            dest.writeInt(-1);
        }
        dest.writeString(this.recordedEntryId);
        dest.writeInt(this.pushPublishEnabled == null ? -1 : this.pushPublishEnabled.ordinal());
        if(this.publishConfigurations != null) {
            dest.writeInt(this.publishConfigurations.size());
            dest.writeList(this.publishConfigurations);
        } else {
            dest.writeInt(-1);
        }
        dest.writeValue(this.firstBroadcast);
        dest.writeValue(this.lastBroadcast);
        dest.writeValue(this.currentBroadcastStartTime);
        ...
        dest.writeBoolean(this.explicitLive);
    ...
    public LiveEntry(Parcel in) {
        super(in);
        this.offlineMessage = in.readString();
        int tmpRecordStatus = in.readInt();
        this.recordStatus = tmpRecordStatus == -1 ? null : RecordStatus.values()[tmpRecordStatus];
        int tmpDvrStatus = in.readInt();
        this.dvrStatus = tmpDvrStatus == -1 ? null : DVRStatus.values()[tmpDvrStatus];
        this.dvrWindow = (Integer)in.readValue(Integer.class.getClassLoader());
        this.lastElapsedRecordingTime = (Integer)in.readValue(Integer.class.getClassLoader());
        int liveStreamConfigurationsSize = in.readInt();
        if( liveStreamConfigurationsSize > -1) {
            this.liveStreamConfigurations = new ArrayList<>();
            in.readList(this.liveStreamConfigurations, LiveStreamConfiguration.class.getClassLoader());
        }
        this.recordedEntryId = in.readString();
        int tmpPushPublishEnabled = in.readInt();
        this.pushPublishEnabled = tmpPushPublishEnabled == -1 ? null : LivePublishStatus.values()[tmpPushPublishEnabled];
        int publishConfigurationsSize = in.readInt();
        if( publishConfigurationsSize > -1) {
            this.publishConfigurations = new ArrayList<>();
            in.readList(this.publishConfigurations, LiveStreamPushPublishConfiguration.class.getClassLoader());
        }
        ...
        this.explicitLive = in.readBoolean();
