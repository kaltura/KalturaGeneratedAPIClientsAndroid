...
public class RenderCaptionAttributes extends CaptionAttributes {
    ...
    public static final Creator<RenderCaptionAttributes> CREATOR = new Creator<RenderCaptionAttributes>() {
        @Override
        public RenderCaptionAttributes createFromParcel(Parcel source) {
            return new RenderCaptionAttributes(source);
        }

        @Override
        public RenderCaptionAttributes[] newArray(int size) {
            return new RenderCaptionAttributes[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeValue(this.fontSize);
        dest.writeString(this.primaryColour);
        dest.writeInt(this.borderStyle == null ? -1 : this.borderStyle.ordinal());
        dest.writeString(this.backColour);
        dest.writeString(this.outlineColour);
        dest.writeValue(this.shadow);
        dest.writeValue(this.bold);
        dest.writeValue(this.italic);
        dest.writeValue(this.underline);
        dest.writeInt(this.alignment == null ? -1 : this.alignment.ordinal());
        dest.writeString(this.captionAssetId);
    }

    public RenderCaptionAttributes(Parcel in) {
        super(in);
        this.fontName = in.readString();
        this.fontSize = (Integer)in.readValue(Integer.class.getClassLoader());
        this.primaryColour = in.readString();
        int tmpBorderStyle = in.readInt();
        this.borderStyle = tmpBorderStyle == -1 ? null : BorderStyle.values()[tmpBorderStyle];
        this.backColour = in.readString();
        this.outlineColour = in.readString();
        this.shadow = (Integer)in.readValue(Integer.class.getClassLoader());
        ...
        int tmpAlignment = in.readInt();
        this.alignment = tmpAlignment == -1 ? null : CaptionsAlignment.values()[tmpAlignment];
